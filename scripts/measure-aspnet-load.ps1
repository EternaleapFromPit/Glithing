...
$source = Join-Path $repo "examples\aspnet_load_test.cs"
$csproj = Join-Path $repo "examples\aspnet_load_test.csproj"
$glExecutable = Join-Path $target "aspnet-load-test-gl.exe"
$csExecutable = Join-Path $repo "examples\bin\Release\net10.0\aspnet_load_test.exe"
...
$glResults + $csResults | Format-Table -Property Compiler, Run, Requests, LiveAllocExit, MaxWorkingSetMB, TimeMS -AutoSize

# 6. Safety check
$failed = ($glResults + $csResults) | Where-Object {
    $_.LiveAllocExit -ne 0 -or $_.Requests -ne "2000000"
}
